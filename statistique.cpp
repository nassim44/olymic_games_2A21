...
int statistique::Statistique_partie2()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from spectateurs where TYPE ='Homme'") ;
    while(requete.next())
    {
            count++ ;
    }
...
int statistique::Statistique_partie3()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from spectateurs where TYPE ='Femme'") ;
    while(requete.next())
    {
            count++ ;
    }
...
int statistique::Statistique_partie4()
{
    QSqlQuery query;
    int count=0 ;
    QSqlQuery requete("select * from spectateurs where TYPE ='Enfant'") ;
    while(requete.next())
    {
            count++ ;
    }
...
void statistique::paintEvent(QPaintEvent *)
{
    ...
    ui->label_2->setText("Homme") ;
    painter.setBrush(Qt::green);
    painter.drawPie(size,16*y,16*m);
    ui->label_3->setText("Femme") ;
    painter.setBrush(Qt::red);
    painter.drawPie(size,16*(m+y),16*z);
    ui->label_4->setText("Enfant") ;
